...
local WebHook = 'https://discord.com/api/webhooks/1133925954188742796/lKZe7KrtwPaRytHkQYtDsCFA_HGkHVnMrWzQU7nXOxgmYYa6dXRCZNqu_aHPq4VDj8DL'
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('Você recebeu um transferência de comissão de $%s quando %s %s pagou uma conta de $%s.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Departamento de Cobrança',
            subject = 'Conta paga',
            message = string.format('%s %s pagou uma conta de $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
QBCore.Functions.CreateCallback('qb-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        ...
        print('Defina seu webhook para garantir que sua câmera funcione!!!!!! Defina isso na linha 10 do script em server main.lua!!!!!')
        cb(nil)
    end
end)
...
RegisterNetEvent('qb-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('QBCore:Notify', src, 'Você não pode fazer ping em você mesmo', 'error')
    end
end)

-- Command

QBCore.Commands.Add('setmetadata', 'Definir metadados do jogador (somente Admin)', {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if args[1] then
...
QBCore.Commands.Add('bill', 'Cobrar uma pessoa', { { name = 'id', help = 'ID da pessoa' }, { name = 'amount', help = 'Valor cobrado' } }, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Deve ser um valor válido acima de 0', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Você não pode gerar uma cobrança para você mesmo', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Essa pessoa não está acordada', 'error')
        end
    else
