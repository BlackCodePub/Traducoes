            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Você já está em uma chamada!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa está ocupada!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa não está disponível!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Você não pode ligar para você!");
        }
    });
});
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Hmm, eu não deveria ser capaz de fazer isso...'
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefone", "Você não pode usar o WhatsApp para si mesmo..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, eu não deveria ser capaz de fazer isso...'
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, eu não deveria ser capaz de fazer isso...'
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contato", "Preencha todos os campos!");
    }
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa está em uma chamada!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa não está disponível!");
            }
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Você recebeu uma chamada de "+CallData.name
                if (AnonymousCall) {
                    Label = "Você está sendo chamado por uma pessoa anônima"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            $(".phone-currentcall-title").html("Chamada realizada");
