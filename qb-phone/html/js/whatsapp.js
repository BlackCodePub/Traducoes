...
QB.Phone.Functions.GetLastMessage = function(messages) {
    ...
    $.each(messages[messages.length - 1], function(i, msg){
        ...
        if(LastMessageData.message == '') 'Hmm, eu não deveria ser capaz de fazer isso...'
    });
...
const monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julio", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Você não pode enviar uma mensagem vazia!", "#25D366", 1750);
    }
});
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.message == '') message.message = 'Hmm, eu não deveria ser capaz de fazer isso...'
                var Sender = "me";
