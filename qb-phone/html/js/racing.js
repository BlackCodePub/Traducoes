...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "A corrida já começou..", "#1DA1F2");
    }
});
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Iniciada';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "SPRINT"
            ...
            var Buttons = '<div class="race-buttons"> <div class="race-button" id="join-race" data-toggle="racetooltip" data-placement="left" title="Participar"><i class="fas fa-sign-in-alt"></i></div>';
            if (InRace) {
                if (!Creator) {
                    ...
                    if (!race.RaceData.Started) {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="start-race" data-toggle="racetooltip" data-placement="left" title="Iniciar"><i class="fas fa-flag-checkered"></i></div><div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Sair"><i class="fas fa-sign-out-alt"></i></div>';
                    } else {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Sair"><i class="fas fa-sign-out-alt"></i></div>';
                    }
                }
                ...
                if (InDistance) {
                    $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você está em um editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você já está em uma corrida..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://qb-phone/GetTrackData', JSON.stringify({
        ...
        $(".racing-setup-information-distance").html('Distância: '+TrackData.Distance+' m');
        ...
        if (TrackData.Records.Holder !== undefined) {
            ...
            $(".racing-setup-information-wr").html('Recorde: ' + secondsTimeSpanToHMS(TrackData.Records.Time) + ' ('+Holder+')');
        } else {
            $(".racing-setup-information-wr").html('Recorde: N/A');
        }
    });
            ...
            if (!data.IsBusy) {
                $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você está em uma corrida..", "#1DA1F2");
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você já está configurando uma corrida..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você não tem permissão para fazer pistas de corrida..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        if (TrackName == '') TrackName = 'O que você está tentando?'
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            ...
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Este nome não está disponível..", "#1DA1F2");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você não tem permissão para criar pistas de corrida..", "#1DA1F2");
            }
        });
    } else {
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você tem que digitar um nome da corrida..", "#1DA1F2");
    }
});
                            ...
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Selecione uma corrida');
                                        $(".racing-setup-information-creator").html('Selecione uma corrida');
                                        $(".racing-setup-information-wr").html('Selecione uma corrida');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Selecione uma corrida");
                                    });
                                } else {
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Não pode haver nenhum ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Preencha uma quantidade de voltas..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você não selecionou uma corrida..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Você já tem uma corrida ativa..", "#1DA1F2");
        }
    });
});
        ...
        if (Races !== null) {
            ...
            $.each(Races, function(i, race){
                if (race.LastLeaderboard.length > 0) {
                    var elem = '<div class="racing-leaderboard-item" id="leaderboard-item-'+i+'"> <span class="racing-leaderboard-item-name"><i class="fas fa-flag-checkered"></i> '+race.RaceName+'</span> <span class="racing-leaderboard-item-info">Clique para mais detalhes</span> </div>'
