...
function DisableDisplayControlActions() -- By Juliroo :)
...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Você não tem um telefone", 'error')
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Telefone',
                text = "Você não recebeu nenhuma chamada...",
                icon = 'fas fa-phone',
                color = '#e84118',
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify('Ação não disponível no momento...', 'error')
        end
    end
end)
...
RegisterNUICallback('SharedLocation', function(data, cb)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'Whatsapp',
            text = 'A localização foi definida!',
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify('Seu veículo foi marcado', 'success')
    else
...
RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    ...
    QBCore.Functions.Notify('Localização GPS definida: ' .. data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Você está muito longe da corrida. O GPS foi definido para a corrida.', 'error', 5000)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS foi configurado!', 'success')
    cb('ok')
end)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('qb-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
                ...
                SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { timeout = 3000, title = 'Banco', text = 'A conta não existe!', icon = 'fas fa-university', color = '#ff0000', }, })
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            QBCore.Functions.TriggerCallback('qb-phone:server:GetWebhook', function(hook)
                if not hook then
                    QBCore.Functions.Notify('Câmera não configurada', 'error')
                    return
                end
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        QBCore.Functions.Notify('Você precisa inserir o ID de uma pessoa', 'error')
    else
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'Banco da Fronteira', text = '&#36;' .. amount .. ' foi adicionado à sua conta!', icon = 'fas fa-university', color = '#8c7ae6', }, })
        ...
        if src ~= MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    ...
                    text = 'Um novo tweet foi postado.',
                    icon = 'fab fa-twitter',
                    color = '#1DA1F2',
                },
            })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'E-mail',
            text = 'Você recebeu um novo e-mail de ' .. MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'Classificados',
            text = 'Um novo anúncio foi publicado por ' .. LastAd,
            icon = 'fas fa-ad',
            color = '#ff8f1a',
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:CancelCall', function()
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({
            ...
            NotifyData = {
                title = 'Telefone',
                content = 'A chamada foi encerrada',
                icon = 'fas fa-phone',
                timeout = 3500,
                color = '#e84118',
            },
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = 'Whatsapp',
                        text = 'Enviou uma mensagem para você mesmo',
                        icon = 'fab fa-whatsapp',
                        color = '#25D366',
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Banco',
                text = '$' .. amount .. ' foi removido da sua conta!',
                icon = 'fas fa-university',
                color = '#ff002f',
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'Telefone',
            text = 'Você tem uma nova sugestão de contato!',
            icon = 'fa fa-phone-alt',
            color = '#04b543',
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Telefone',
                text = "Você não tem nenhuma chamada recebida...",
                icon = 'fas fa-phone',
                color = '#e84118',
            },
        })
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.Notify('Ninguém por perto!', 'error')
    end
end)
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'Você foi mencionado em um Tweet!', text = TweetMessage.message, icon = 'fab fa-twitter', color = '#1DA1F2', }, })
